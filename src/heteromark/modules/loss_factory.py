...
class LossFactory(BaseLossFactory):
    ...
    def _create_happo_loss(
        ...
        advantage_module, critic_loss_module = self._create_advantag_module(
            config, value_module
        )
    ...
    def _create_advantag_module(self, config, value_module) -> tuple:
        # clip_epsilon = config.get("clip_epsilon", 0.2)
        # entropy_coeff = config.get("entropy_coeff", 0.01)
        critic_coeff = config.get("critic_coeff", 1.0)
        gamma = config.get("gamma", 0.99)
        ...
        return advantage_module, critic_loss_module
...
def get_dummy_ppo_loss_modules_from_factory(policy_modules, value_modules):
...
def get_dummy_happo_loss_modules_from_factory(policy_modules, value_modules):
    loss_factory = LossFactory("happo")
    config = {}
    loss_modules, advantage_modules = loss_factory.create(
        config=config, policy_modules=policy_modules, value_modules=value_modules
    )
